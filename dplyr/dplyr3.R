...
students %>%
  ...
  summarise(
    first_grade_mean = mean(grade1),
    second_grade_mean = mean(grade2)
    )
...
students %>%
  ...
  summarise(studytime_average = mean(study_time))
...
students %>%
  group_by(sex) %>%
  summarize(failures_average = mean(failures))
...
students %>%
  group_by(romantic) %>%
  summarise(final_grade_average = mean(final_grade))
