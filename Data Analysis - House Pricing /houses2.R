...
#'
#' When one bedroom increase, the house is expected to sell for 18.7%. 
#' That estimate is significant at the 1% level.
#'
#'
...
#'
#' When bedroom increases by 1%, the selling price is expected to increase 
#' by 2.95%, all others held constant. That estimate is significant at the 
#' 1% level.
#'
#'
#' # 3. Multicollinearity
...
#'
#' The 3 variables `bedrooms` is most correlated with are stories, bathrooms, and
#' price.
#' 
...
houses %>%
  mutate(
  bedrooms_resid = residuals(lm(bedrooms ~ ., data = select(houses,-price))),
      log_price_resid = residuals(lm(log(price) ~ . , data = select(houses, -bedrooms)))) %>%
...
#' 
#' #2 would be the best way of fixing the problem causes by multicollinearity. Since
#' multicollinearity occurs when two or more x variables correlated with each others.
#' When combining the correlated variables into an overall index, which is one 
#' variable, that minimize the chance of having some correlated variables in the model.
#' Thus, #2 would be possible to fix multicollinearity.
